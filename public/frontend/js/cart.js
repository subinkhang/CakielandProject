...
for (let i = index; i < products.length; i++) {
    const nextItem=document.getElementById(`item${i + 1}`);
    if (nextItem) {
    nextItem.id=`item${i}`;
    nextItem.querySelector(".removeitem").setAttribute("onclick", `deleteItem(${i})`);
    nextItem.querySelector(".price").setAttribute("name", `price[${i}]`);
    nextItem.querySelector(".subprice").id=`subprice_${i}`;
    nextItem.querySelector("span input").name=`quan[${i}]`;
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        newRow.id=`item${i}`;
        const subtotal=products[i].price * products[i].quantity; const price=parseFloat(products[i].price);
        console.log(price);
