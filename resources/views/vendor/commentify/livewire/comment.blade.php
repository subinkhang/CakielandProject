<div>
    ...
    @else
        <article class="p-6 mb-1 text-base bg-white rounded-lg dark:bg-gray-900 shadow-md">
            <footer class="flex justify-between items-center mb-1">
                <div class="flex items-center">
                    <img class="mr-2 w-8 h-8 rounded-full" src="{{$comment->user->avatar()}}" alt="{{$comment->user->name}}">
                    <div>
                        <p class="inline-flex items-center text-sm text-gray-900 dark:text-white">{{Str::ucfirst($comment->user->name)}}</p>
                    ...
                    @if($showOptions)
                        <div class="absolute z-10 top-full right-0 mt-1 w-36 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                @can('update',$comment)
                                    <li>
                                        <button wire:click="$toggle('isEditing')" type="button" class="block w-full text-left py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</button>
                                    </li>
                                @endcan
                                @can('destroy',$comment)
                                    <li>
                                        <button x-on:click="confirmCommentDeletion" x-data="{ confirmCommentDeletion(){ if(window.confirm('You sure to delete this comment?')){ @this.call('deleteComment') } }}" class="block w-full text-left py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Delete</button>
                                    </li>
                                @endcan
                            </ul>
                        </div>
